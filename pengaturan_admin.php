...
<head>
    <meta charset="UTF-8">
    <title>MadiunTrack - Pengaturan Sistem</title>
...
<?php
// ===== Dummy data pengaturan & riwayat backup =====
$pengaturan = [
    'nama_situs' => 'MadiunTrack',
    'email_kontak' => 'admin@madiuntrack.id',
    'no_telp' => '0351-123456',
    'alamat' => 'Jl. Pahlawan No. 1, Kota Madiun',
];

$riwayat_backup = [
    ['tgl' => '04 Jul 2026 - 21:10', 'jenis' => 'Data Pengguna', 'jumlah' => '1.256 baris', 'ukuran' => '340 KB', 'oleh' => 'Admin'],
    ['tgl' => '01 Jul 2026 - 08:45', 'jenis' => 'Data Tiket Wisata', 'jumlah' => '842 baris', 'ukuran' => '210 KB', 'oleh' => 'Admin'],
    ['tgl' => '28 Jun 2026 - 17:22', 'jenis' => 'Data Reservasi Homestay', 'jumlah' => '204 baris', 'ukuran' => '95 KB', 'oleh' => 'Admin'],
];
?>
        <div class="p-4 md:p-7">

            <h1 class="text-2xl font-semibold m-0 mb-1 text-slate-900">Pengaturan Sistem</h1>
            <p class="text-slate-500 text-sm m-0 mb-6">Atur informasi situs serta kelola backup data yang berasal dari halaman pengguna.</p>

            <div class="grid grid-cols-1 xl:grid-cols-[1.2fr_1fr] gap-4 items-start">

                <!-- FORM PENGATURAN UMUM -->
                <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                    <div class="flex items-center gap-2.5 font-semibold text-[15px] mb-4">
                        <span class="w-9 h-9 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center"><?= icon('settings', 16) ?></span>
                        Informasi Umum Situs
                    </div>
                    <div class="flex flex-col gap-4">
                        <div>
                            <label class="text-[12.5px] font-semibold text-slate-600 block mb-1.5">Nama Situs</label>
                            <input type="text" value="<?= htmlspecialchars($pengaturan['nama_situs']) ?>" class="w-full text-[13.5px] border border-slate-200 rounded-lg px-3 py-2.5 outline-none focus:border-blue-400">
                        </div>
                        <div>
                            <label class="text-[12.5px] font-semibold text-slate-600 block mb-1.5">Email Kontak</label>
                            <input type="email" value="<?= htmlspecialchars($pengaturan['email_kontak']) ?>" class="w-full text-[13.5px] border border-slate-200 rounded-lg px-3 py-2.5 outline-none focus:border-blue-400">
                        </div>
                        <div>
                            <label class="text-[12.5px] font-semibold text-slate-600 block mb-1.5">No. Telepon</label>
                            <input type="text" value="<?= htmlspecialchars($pengaturan['no_telp']) ?>" class="w-full text-[13.5px] border border-slate-200 rounded-lg px-3 py-2.5 outline-none focus:border-blue-400">
                        </div>
                        <div>
                            <label class="text-[12.5px] font-semibold text-slate-600 block mb-1.5">Alamat</label>
                            <textarea rows="2" class="w-full text-[13.5px] border border-slate-200 rounded-lg px-3 py-2.5 outline-none focus:border-blue-400"><?= htmlspecialchars($pengaturan['alamat']) ?></textarea>
                        </div>
                        <button class="self-start mt-1 py-2.5 px-5 rounded-xl font-semibold text-[13px] text-white bg-blue-700 hover:bg-blue-800 transition-colors cursor-pointer">Simpan Perubahan</button>
                    </div>
                </div>

                <!-- BACKUP & EKSPOR DATA USER -->
                <div class="flex flex-col gap-4">
                    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                        <div class="flex items-center gap-2.5 font-semibold text-[15px] mb-1">
                            <span class="w-9 h-9 rounded-xl bg-green-100 text-green-600 flex items-center justify-center"><?= icon('download', 16) ?></span>
                            Backup Data dari Pengguna
                        </div>
                        <p class="text-[12.5px] text-slate-500 leading-relaxed mb-4">Ambil dan simpan salinan (backup) data yang berasal dari aktivitas pengguna di halaman user.</p>
                        <div class="flex flex-col gap-2.5">
                            <button class="flex items-center justify-between py-2.5 px-4 rounded-xl font-semibold text-[13px] text-slate-700 bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-colors cursor-pointer">
                                <span class="flex items-center gap-2"><?= icon('ticket', 15) ?> Backup Data Tiket Wisata</span>
                                <?= icon('download', 14) ?>
                            </button>
                            <button class="flex items-center justify-between py-2.5 px-4 rounded-xl font-semibold text-[13px] text-slate-700 bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-colors cursor-pointer">
                                <span class="flex items-center gap-2"><?= icon('building', 15) ?> Backup Data Reservasi Homestay</span>
                                <?= icon('download', 14) ?>
                            </button>
                            <button class="flex items-center justify-between py-2.5 px-4 rounded-xl font-semibold text-[13px] text-slate-700 bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-colors cursor-pointer">
                                <span class="flex items-center gap-2"><?= icon('user', 15) ?> Backup Data Pengguna</span>
                                <?= icon('download', 14) ?>
                            </button>
                            <button class="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl font-semibold text-[13px] text-white bg-slate-800 hover:bg-slate-900 transition-colors cursor-pointer mt-1">
                                <?= icon('file-text', 15, '#fff') ?> Backup Semua Data Sekaligus
                            </button>
                        </div>
                    </div>

                    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                        <div class="font-semibold text-[14px] text-slate-900 mb-3">Riwayat Backup Terakhir</div>
                        <?php foreach ($riwayat_backup as $r): ?>
                            <div class="flex items-center justify-between py-2.5 border-b border-slate-100 last:border-b-0">
                                <div>
                                    <div class="text-[13px] font-medium text-slate-800"><?= htmlspecialchars($r['jenis']) ?></div>
                                    <div class="text-[11.5px] text-slate-500"><?= htmlspecialchars($r['tgl']) ?> &middot; <?= htmlspecialchars($r['jumlah']) ?></div>
                                </div>
                                <span class="text-[11.5px] font-semibold text-slate-500"><?= htmlspecialchars($r['ukuran']) ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
